...
- [x] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  1. Handled locally in browser by javascript
  2. No refresh on requests, uses AJAX 
  3. Only certain data changes and rest can remain the same

  - Server-Side Routing:

  1. Handled from server to your browser
  2. Refresh on every request 
  3. You re-request data for things you already have rendered on page becuase whole new page has to load.
